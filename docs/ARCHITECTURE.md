...
4. 삭제 전 countdown / 취소 확인
5. MAC별 삭제 명령 한 번 전송
6. 전체 삭제 단계 종료 후 검증 조회
7. 결과 상태 갱신
8. 감사 요약·이력 저장
...
`MmSession`은 Netmiko 기반 MM 연결을 담당합니다. GUI에서는 같은 접속 정보에 대해 세션을 재사용할 수 있지만, 변경 명령의 재시도 여부는 상위 Runner가 명시적으로 제어합니다.
